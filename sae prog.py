...
for bloc in films_bruts:
    if len(bloc) > 10:
        ...
        if len(parties) >= 2:
            ...
            # --- NOUVEAUTÉ : Nettoyage avec N/A pour les cellules vides ---
            titre = titre_brut.strip().title() if titre_brut.strip() else "N/A"
            restriction = restriction_brut.strip().title() if restriction_brut.strip() else "N/A"
            description = desc_brut.strip().replace('"', "'") if desc_brut.strip() else "N/A"
            
            annee = str(int(float(annee_brut))) if annee_brut.strip() else "N/A"
            duree = str(int(float(duree_brut))) if duree_brut.strip() else "N/A"
            votes = votes_brut.strip() if votes_brut.strip() else "N/A"
            score = score_brut.strip() if score_brut.strip() else "N/A"
        
            # Si le score n'est pas N/A, on l'ajoute pour les statistiques
            if score != "N/A":
            ...
            ref = "F-" + code_groupe + str(compteur_films) + "-" + (annee if annee != "N/A" else "0000")
            ligne = f'"{ref}","{titre}","{duree}","{annee}","{restriction}","{votes}","{score}","{description}"'
...
f = open(chemin_csv, mode="w", encoding="utf-8")
f.write("\n".join(lignes_csv))
f.close()
