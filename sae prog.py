...
# On prépare la première ligne du futur fichier Excel (CSV) avec des virgules et des guillemets
...
for bloc in films_bruts:
    # On vérifie que le bloc contient bien des informations
    if len(bloc) > 10:
        ...
        if len(parties) >= 2:
            ...
            # ATTENTION : On enlève les guillemets potentiels dans la description pour ne pas casser le CSV
            description = desc_brut.strip().replace('"', "'")
            ...
            # Création de la ligne finale : on utilise des virgules comme séparateur
            # et on entoure chaque variable de guillemets
...
# CORRECTION : On passe l'encodage en "utf-8" pour GitHub
